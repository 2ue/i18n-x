...
const errors = [{
  type: 'validation',
  message: $t1('yong_hu_ming_wu_xiao')
}, {
  type: 'network',
  message: $t1('wang_luo_cuo_wu')
}, {
  type: 'validation',
  message: $t1('mi_ma_tai_duan')
}];
const messages = errors.filter(err => err.type === 'validation').map(err => `${$t1('yan_zheng_cuo_wu_gyzda6')}：${err.message}`).join('，');

// Set和Map
const statusSet = new Set([$t1('dai_chu_li'), $t1('chu_li_zhong'), $t1('yi_wan_cheng')]);
const errorMap = new Map([['404', $t1('ye_mian_wei_zhao_dao')], ['500', $t1('fu_wu_qi_cuo_wu')]]);
...
// 多维数组
const matrix = [[$t1('di_yi_hang'), $t1('di_yi_lie')], [$t1('di_er_hang'), $t1('di_er_lie')]];

// 数组方法组合
const result = users.filter(user => user.id > 1).map(user => ({
  ...user,
  label: `${$t1('yong_hu')}：${user.name}`
})).reduce((acc, user) => {
