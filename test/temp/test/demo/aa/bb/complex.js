...
const templateComplex = `${$t1('yong_hu')}${userName}${$t1('nian_ling')}${age}岁`;
...
const complexArray = [{
  id: 1,
  title: $t1('wen_zhang_biao_ti'),
  content: `${$t1('nei_rong_zhai_yao')}${Math.random()}`
}, {
  id: 2,
  title: $t1('xin_wen_biao_ti'),
  content: $t1('xin_wen_nei_rong_xiang_qing')
}];
...
const calculated = `${$t1('zong_ji_2vetb')}：${10 + 20}元`;
...
const buildComplexMessage = (user, action) => {
  ...
  return `${time} - ${$t1('yong_hu')}"${user}"${$t1('zhi_xing_le_3j9lfs')}${prefix}${$t1('cao_zuo')}`;
};
...
// 14. 数值和字符串混合
const priceMessage = `${$t1('shang_pin_jia_ge_14ydpe')}：￥${99.99}元`;
