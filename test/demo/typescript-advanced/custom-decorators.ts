...
function Measure(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
  ...
  descriptor.value = async function (...args: any[]) {
    ...
    try {
      ...
      console.log(`${propertyKey} 方法执行完成，耗时: ${endTime - startTime}ms`);
      return result;
    } catch (error) {
      console.error(`${propertyKey} 方法执行失败: ${error.message}`);
      throw error;
    }
  };
...
class UserService {
  ...
  updateProfile(@Required name: string, age: number, @Required role: string): string {
    return `用户资料已更新: 姓名=${name}, 年龄=${age}, 角色=${role}`;
  }
...
function Deprecated(message: string = '此方法已过时') {
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: any[]) {
      console.warn(`警告: ${propertyKey} 方法已被弃用. ${message}`);
      return originalMethod.apply(this, args);
    };
    ...
    descriptor.value = async function (...args: any[]) {
      ...
      if (cache.has(key)) {
        ...
        if (now - cacheEntry.timestamp < ttl) {
          console.log(`从缓存获取 ${propertyKey} 结果`);
      ...
      console.log(`缓存 ${propertyKey} 结果，有效期 ${ttl}ms`);
...
class DataService {
  ...
  async fetchData(id: number): Promise<object> {
    ...
    return { id, name: '数据项', timestamp: Date.now() };
  }

  @Deprecated('请使用 fetchDataV2 方法代替')
  @Measure
  async fetchLegacyData(): Promise<object> {
    await new Promise(resolve => setTimeout(resolve, 500));
    return { legacy: true, data: '旧数据' };
  }
...
function createLogDecorator(options: {
  ...
  return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    descriptor.value = function (...args: any[]) {
      ...
      console.log(`${prefix}调用 ${propertyKey} 方法，参数:`, args);
      const result = originalMethod.apply(this, args);
      console.log(`${prefix}${propertyKey} 方法返回:`, result);
...
// 使用工厂装饰器
class ConfigService {
  @createLogDecorator({ level: '信息', includeTimestamp: true })
  getConfig(name: string): object {
    return { name, value: `配置值-${name}`, enabled: true };
  }

  @createLogDecorator({ level: '警告' })
  resetConfig(name: string): boolean {
    console.log(`重置配置: ${name}`);
    return true;
  }
}
...
class AnalyticsService {
  ...
  processData(data: any[]): object {
    console.log(`处理 ${data.length} 条数据记录`);
    return { processed: data.length, status: '完成' };
  }
}
...
async function demonstrateDecorators() {
  // 用户服务示例
  const userService = new UserService('管理员用户');
  await userService.fetchUserData();

  try {
    userService.updateProfile('张三', 30, '管理员');
  } catch (error) {
  ...
  configService.getConfig('主题');
  configService.resetConfig('缓存');
